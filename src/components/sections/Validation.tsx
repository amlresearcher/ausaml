...
type Tab = 'siv' | 'srv'
type SIVStatus = 'pass' | 'warn' | 'skip'

// ── SIV data ────────────────────────────────────────────────────────────────

interface SIVCheck { id: string; name: string; status: SIVStatus; threshold: string; notes: string }
interface SIVCategory { label: string; checks: SIVCheck[] }

const SIV_CATEGORIES: SIVCategory[] = [
  {
  ...
  },
  {
    ...
    checks: [
      ...
      { id: 'E4.15', status: 'pass', name: 'Intrabank hub: receives ≥2 inbound and fans out ≥2 outbound transfers', threshold: '≥70% of intrabank_hub scenarios have a hub account with ≥2 in and ≥2 out', notes: '3/3 scenarios have a qualifying hub account (100%).' },
  ...
  },
  {
    ...
    checks: [
      ...
      { id: 'E5.3', status: 'pass', name: 'Transaction volume shows natural seasonal variation', threshold: 'CV of monthly counts > 0.05', notes: 'CV=0.354 across 7 months.' },
      ...
      { id: 'E5.6', status: 'pass', name: 'PayID usage proportion is within the AU expected range', threshold: '7–15% of individual deposit account transactions', notes: '466,645/5,490,579 transactions are PayID (8.5%).' },
    ],
  },
  {
    ...
    checks: [
      ...
      { id: 'E6.2', status: 'warn', name: 'Self-employed customers have more variable transaction amounts', threshold: 'Self-employed median CV ≥ 1.5× full-time', notes: 'self_employed CV=0.143 (n=1,956), full_time CV=0.177 (n=4,578); ratio=0.81×.' },
  ...
  },
  {
    ...
    checks: [
      ...
      { id: 'E7.6', status: 'warn', name: 'Home loan mortgages paid off before the borrower turns 70', threshold: 'p99 of (age_at_open + term_years) ≤ 70 years', notes: 'p99 projected age at maturity = 71.0 years.' },
  ...
  },
  {
    ...
    checks: [
      { id: 'E13.1', status: 'pass', name: 'Every account is linked to an existing customer', threshold: '0 orphaned accounts', notes: '0 of 28,140 accounts have no customer_master row.' },
      { id: 'E13.2', status: 'pass', name: 'Every transaction is linked to an existing account', threshold: '0 orphaned transactions', notes: '0 of 8,885,318 transactions have no account_base row.' },
...
const SIV_SUMMARY = { total: 101, passed: 84, warned: 10, skipped: 7 }

// ── SRV data ────────────────────────────────────────────────────────────────

interface SRVCheck { id: string; name: string; threshold: string; interpretation: string }
interface SRVCategory { label: string; checks: SRVCheck[] }

const SRV_CATEGORIES: SRVCategory[] = [
  {
    label: 'Statistical Fidelity',
    checks: [
      { id: 'C1.1', name: "Benford's Law Conformance", threshold: 'MAD < 0.015', interpretation: "MAD=0.0115 — first-digit distribution conforms to Benford's Law." },
      { id: 'C1.2', name: 'Transaction Amount Heavy Tail', threshold: 'P90/P10 > 10', interpretation: 'P90=$1,114.68, P10=$10.86, ratio=102.64 — heavy tail confirmed.' },
      { id: 'C1.3', name: 'Income Distribution Skewness', threshold: 'skewness > 0.5', interpretation: 'Skewness=4.407 — right-skewed income distribution consistent with real data.' },
      { id: 'C1.4', name: 'Cash Transaction Proportion', threshold: '5%–20%', interpretation: '829,750 / 8,840,512 = 9.39% — within expected AU cash usage range.' },
      { id: 'C1.5', name: 'International Wire Proportion', threshold: '0.5%–5%', interpretation: '159,279 / 8,840,512 = 1.80% — within expected range.' },
    ],
  },
  {
    label: 'Temporal Fidelity',
    checks: [
      { id: 'C2.1', name: 'Weekday vs Weekend Volume Ratio', threshold: 'weekday avg ≥ 1.5× weekend avg', interpretation: 'Avg weekday=50,840 txns/day, weekend=19,589 txns/day — ratio=2.60× confirmed.' },
      { id: 'C2.2', name: 'Business Hours Concentration', threshold: '≥ 60% between 07:00–22:00', interpretation: '80.0% of POS/ATM transactions in business hours — realistic daytime concentration.' },
      { id: 'C2.3', name: 'Monthly Volume Coefficient of Variation', threshold: 'CV > 0.08', interpretation: 'Monthly CV=0.382 across 7 months — sufficient seasonal variation.' },
      { id: 'C2.4', name: 'Payroll Periodicity', threshold: 'median gap std-dev ≤ 5 days', interpretation: 'Median payroll gap std-dev=0.00 days across 10,105 customers — regular cycle confirmed.' },
      { id: 'C2.5', name: 'Intraday Rhythm', threshold: 'peak/trough hourly ratio > 2.0', interpretation: 'Peak=473,967 txns, trough=188,787 txns, ratio=2.51 — realistic intraday variation.' },
    ],
  },
  {
    label: 'AML Signal',
    checks: [
      { id: 'C4.1', name: 'AML Transaction Velocity Spike', threshold: 'median ratio > 1.2×', interpretation: 'Median scenario-window / pre-window velocity=1.447× across 200 AML accounts — expected spike confirmed.' },
      { id: 'C4.2', name: 'Structuring Signal in Smurfing Scenarios', threshold: 'fraction > 0.35 (amounts 70–99% of CTR threshold)', interpretation: '35/64 smurfing deposits just below CTR threshold = 54.69% — strong structuring signal.' },
      { id: 'C4.3', name: 'Hard Negative Proximity', threshold: 'clean LR < AML LR; clean risk < suspected risk', interpretation: 'LR: clean=0.041, AML=0.095. KYC risk: clean=23.1, suspected=79.2 — clean separation confirmed.' },
    ],
  },
]

const SRV_SUMMARY = { total: 13, passed: 13 }

// ── Shared styles ────────────────────────────────────────────────────────────

const SIV_STATUS_STYLES: Record<SIVStatus, { card: string; badge: string; label: string }> = {
...
function sivCatStatus(checks: SIVCheck[]): 'pass' | 'warn' {
  return checks.some(c => c.status === 'warn') ? 'warn' : 'pass'
}
...
function SIVView() {
  ...
  const passRate = Math.round((SIV_SUMMARY.passed / SIV_SUMMARY.total) * 100)

  return (
    <div>
      <div className="flex flex-wrap gap-3 mb-8">
        <div className="bg-green-100 text-green-800 text-sm font-semibold px-4 py-2 rounded-lg">{SIV_SUMMARY.passed} passed</div>
        <div className="bg-amber-100 text-amber-800 text-sm font-semibold px-4 py-2 rounded-lg">{SIV_SUMMARY.warned} warned</div>
        <div className="bg-slate-200 text-slate-600 text-sm font-semibold px-4 py-2 rounded-lg">{SIV_SUMMARY.skipped} skipped</div>
      ...
      <div className="space-y-2">
        {SIV_CATEGORIES.map(cat => {
          const st = sivCatStatus(cat.checks)
          const isOpen = expanded.has(cat.label)
          const countByStatus = (s: SIVStatus) => cat.checks.filter(c => c.status === s).length
          return (
            <div key={cat.label} className="border border-slate-200 rounded-lg overflow-hidden">
              <button
                onClick={() => toggle(cat.label)}
                className="w-full flex items-center justify-between gap-3 px-5 py-3.5 bg-white hover:bg-slate-50 transition-colors text-left"
              >
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="font-semibold text-navy-900 text-sm">{cat.label}</span>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${st === 'warn' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>
                    {st === 'warn' ? 'WARN' : 'PASS'}
                  </span>
                ...
                <span className="text-slate-400 shrink-0 text-sm">{isOpen ? '▾' : '▸'}</span>
              </button>
                ...
                <div className="p-4 bg-slate-50 border-t border-slate-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {cat.checks.map(check => {
                      const s = SIV_STATUS_STYLES[check.status]
                      return (
                        <div key={check.id} className={`border rounded-lg p-4 ${s.card}`}>
                          <div className="flex items-start justify-between gap-2 mb-2">
                            ...
                            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full shrink-0 ${s.badge}`}>{s.label}</span>
                          </div>
...
function SRVView() {
  return (
    <div className="space-y-8">
      {SRV_CATEGORIES.map(cat => (
        <div key={cat.label}>
          <h3 className="text-base font-semibold text-navy-900 mb-4 flex items-center gap-3">
            {cat.label}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {cat.checks.map(check => (
              <div key={check.id} className="border border-green-200 bg-green-50 rounded-lg p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <span className="font-mono text-xs text-slate-400 shrink-0 mt-0.5">{check.id}</span>
                  <span className="bg-green-100 text-green-700 text-xs font-semibold px-2 py-0.5 rounded-full shrink-0">PASS</span>
                </div>
                <p className="text-sm font-semibold text-navy-900 mb-1 leading-snug">{check.name}</p>
                <p className="text-xs text-slate-500 mb-2">Threshold: {check.threshold}</p>
...
export function Validation() {
  const [tab, setTab] = useState<Tab>('siv')

  return (
    <section id="validation" className="py-20 px-6 bg-slate-50">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-3xl font-bold text-navy-900 mb-1">Dataset Validation</h2>
        ...
        <div className="flex gap-2 mb-8">
          <button
            onClick={() => setTab('siv')}
            className={`px-5 py-2.5 text-sm font-semibold rounded-lg transition-colors ${tab === 'siv' ? 'bg-navy-900 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
          >
            Synthetic Integrity
            <span className={`ml-2 text-xs font-medium px-2 py-0.5 rounded-full ${tab === 'siv' ? 'bg-white/20 text-white' : 'bg-amber-100 text-amber-700'}`}>
              WARN
            </span>
          </button>
          <button
            onClick={() => setTab('srv')}
            className={`px-5 py-2.5 text-sm font-semibold rounded-lg transition-colors ${tab === 'srv' ? 'bg-navy-900 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
          >
            Synthetic Realism
            <span className={`ml-2 text-xs font-medium px-2 py-0.5 rounded-full ${tab === 'srv' ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'}`}>
              {SRV_SUMMARY.passed}/{SRV_SUMMARY.total} PASS
            </span>
          </button>
        </div>
        ...
        {tab === 'siv' ? (
          <div>
            <p className="text-slate-500 text-sm mb-6">
              {SIV_SUMMARY.total} automated checks verifying internal consistency, referential integrity, and AML signal quality.
            </p>
            <SIVView />
          </div>
        ) : (
          <div>
            <p className="text-slate-500 text-sm mb-6">
              {SRV_SUMMARY.total} checks verifying statistical, temporal, and AML signal realism against real-world banking benchmarks.
            </p>
            <SRVView />
          </div>
        )}
